// Vista del carrito.
// - Lista items añadidos, permite ajustar cantidades y eliminar.
// - Muestra el total y botón de "Finalizar" (demo).
  ...
  return (
    <div style={styles.wrap}>
          ...
          <li key={item.id} style={styles.item}>
            ...
            <div style={styles.info}>
              ...
              <div style={styles.controls}>
                <button onClick={() => setQty(item.id, Math.max(item.qty - 1, 1))}>-</button>
                <input
                  type="number"
                  min="1"
                  value={item.qty}
                  onChange={(e) =>
                    setQty(item.id, Math.max(parseInt(e.target.value || '1', 10), 1))
                  }
                  style={styles.input}
                />
            ...
            <div style={styles.subtotal}>{formatCOP(item.price * item.qty)}</div>
          </li>
        ))}
      </ul>
      ...
      <div style={styles.summary}>
        ...
        <button style={styles.checkout} onClick={clearCart}>
